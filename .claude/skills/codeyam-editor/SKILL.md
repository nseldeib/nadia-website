...
Each feature flows through plan → confirm → prepare → prototype → demo → deconstruct → present → reconcile → finalize → journal → commit → push → feature-complete. Run `codeyam-editor-dev editor step 1` to start; subsequent commands tell you the next slug.
...
codeyam-editor-dev editor server-identity
```
